...
def doc2vec_vectorize(filename, vector_size=500, window=2, min_count=15, max_vocab_size=10000, directory='ProcessedWSJ/'):
    corpus = pickle.load(open(filename, 'rb'))
    ...
    start = time.perf_counter()
    model = Doc2Vec(documents, vector_size=500, window=2, min_count=15, max_vocab_size=10000,
