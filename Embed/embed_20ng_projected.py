...
PROCESSED_DIR      = 'Processed20NG'
NEWGROUPS_PICKLE   = os.path.join(PROCESSED_DIR, '20ng_raw.pkl')
...
def main():
    ...
    if os.path.exists(NEWGROUPS_PICKLE):
        ...
        ng20 = NG20()
        documents = ng20.get_all_texts()  # should return List[str]
        with open(NEWGROUPS_PICKLE, 'wb') as f:
            pickle.dump(documents, f)
        print(f"Fetched and cached {len(documents)} 20NG documents.")
    ...
    emb_path = os.path.join(PROCESSED_DIR, '20ng_corpus_embeddings.pkl')
    ...
    proj_path = os.path.join(PROCESSED_DIR, '20ng_projected_features.pkl')
